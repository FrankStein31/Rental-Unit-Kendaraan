...
# Form implementation generated from reading ui file 'tambah_elf.ui'
#
...
"background-color: rgb(212, 17, 30);")
    ...
    def insert_elf_data(self):
        ...
        id_elf = self.lineEdit_id.text()           # Ambil input dari field ID Elf
        jenis_elf = self.lineEdit_jenis.text()        # Ambil input dari field Jenis Elf
        type_elf = self.lineEdit_type.text()         # Ambil input dari field Tipe Elf
        tahun_elf = self.lineEdit_tahun.text()
        warna_elf = self.lineEdit_warna.text()        # Ambil input dari field Warna Elf
        stok_elf = int(self.lineEdit_stok.text())    # Ambil input dari field Stok Elf, pastikan konversi ke int
        ...
        try:
                ...
                sql = """
                INSERT INTO elf (id_elf, jenis_elf, type_elf, tahun_elf, warna_elf, stok_elf, harga_sewa_elf)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
                """
                val = (id_elf, jenis_elf, type_elf, tahun_elf, warna_elf, stok_elf, harga_sewa_elf)
                mycursor.execute(sql, val)

                # Commit perubahan
                conn.commit()
                print("Data elf berhasil ditambahkan...")
