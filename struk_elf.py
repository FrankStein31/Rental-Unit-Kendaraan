...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow, user_id=None):
        ...
        header_label = QtWidgets.QLabel("Struk Transaksi ELF")
        ...
        self.transaction_table.setHorizontalHeaderLabels(["ID", "Jenis ELF", "Tipe ELF", "Tahun", "Warna", "Total Pembayaran"])
    ...
    def load_all_transactions(self):
        try:
            ...
            query = """
                SELECT DISTINCT m.id_elf, m.jenis_elf, m.type_elf, m.tahun_elf, m.warna_elf, pm.total_pembayaran
                FROM elf m
                JOIN pinjam_elf p ON m.id_elf = p.id_elf
                JOIN pembayaran_elf pm ON p.id_transaksi_elf = pm.id_transaksi_elf
                WHERE p.id_user = %s
            """
