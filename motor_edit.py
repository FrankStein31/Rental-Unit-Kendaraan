...
# Form implementation generated from reading ui file 'edit_motor.ui'
#
...
"background-color: rgb(255, 202, 111);")
        ...
        self.pushButton_cari.setGeometry(QtCore.QRect(240, 40, 101, 31))
        self.pushButton_cari.setObjectName("pushButton_cari")
        self.pushButton_cari.setStyleSheet("background-color: rgb(220, 220, 220);\n"
"background-color: rgb(255, 202, 111);")
        self.pushButton_simpan = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_simpan.setGeometry(QtCore.QRect(530, 540, 111, 31))
        ...
        self.pushButton_keluar.setGeometry(QtCore.QRect(660, 540, 111, 31))
...
"background-color: rgb(212, 17, 30);")
    ...
    def cari_motor(self):
        ...
        try:
            ...
            # Query SQL untuk mencari motor berdasarkan ID
            sql = "SELECT * FROM motor WHERE id_mtr = %s"
            val = (id_mtr,)
            ...
            if result:
                # Jika data ditemukan, tampilkan di GUI
                self.lineEdit_jenis.setText(str(result[1]))  # jenis_mtr
                self.lineEdit_type.setText(str(result[2]))  # type_mtr
                self.lineEdit_tahun.setText(str(result[3]))  # tahun_mtr
                self.lineEdit_warna.setText(str(result[4]))  # warna_mtr
                self.lineEdit_stok.setText(str(result[5]))  # stok_mtr
                self.lineEdit_harga.setText(str(result[6]))  # harga_sewa_mtr
            else:
                QtWidgets.QMessageBox.information(self.centralwidget, "Data Tidak Ditemukan", "Motor dengan ID tersebut tidak ditemukan.")
    ...
    def update_motor_data(self):
        ...
        id_mtr = self.lineEdit_id.text()  # Ambil input ID Motor
        jenis_mtr = self.lineEdit_jenis.text()  # Ambil input Jenis Motor
        type_mtr = self.lineEdit_type.text()  # Ambil input Tipe Motor
        tahun_mtr = self.lineEdit_tahun.text()
        warna_mtr = self.lineEdit_warna.text()  # Ambil input Warna Motor
        stok_mtr = int(self.lineEdit_stok.text())  # Ambil input Stok Motor
        harga_sewa_mtr = int(self.lineEdit_harga.text())  # Ambil input Harga Sewa
        ...
        try:
            ...
            sql = """
            UPDATE motor
            SET jenis_mtr = %s, type_mtr = %s, tahun_mtr = %s, warna_mtr = %s, stok_mtr = %s, harga_sewa_mtr = %s
            WHERE id_mtr = %s
            """
            ...
            # Commit perubahan
            conn.commit()
            print("Data motor berhasil diperbarui...")
