...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.central_layout.addWidget(self.motor_table, 2)
        
        MainWindow.setCentralWidget(self.centralwidget)
        
        # Koneksi sinyal
        self.pushButton_pilihfoto.clicked.connect(self.images)
        self.pushButton_cari.clicked.connect(self.cari_motor_by_jenis)
        self.pushButton_simpan.clicked.connect(self.update_motor_data)
        self.pushButton_keluar.clicked.connect(self.KembaliMenu)
        self.pushButton_keluar.clicked.connect(MainWindow.close)
        
        # Tampilkan semua motor saat pertama kali dibuka
        self.load_all_motors()
        
        # Double click pada tabel untuk edit
        self.motor_table.doubleClicked.connect(self.isi_form_dari_tabel)

    def load_all_motors(self):
        try:
            ...
            mycursor.execute("SELECT * FROM motor")
            results = mycursor.fetchall()
            
            self.motor_table.setRowCount(0)
            for row_data in results:
                row_position = self.motor_table.rowCount()
                self.motor_table.insertRow(row_position)
                for i, value in enumerate(row_data):
                    self.motor_table.setItem(row_position, i, QTableWidgetItem(str(value)))
    ...
    def cari_motor_by_jenis(self):
        jenis_mtr = self.lineEdit_jenis.text()
        ...
        try:
            ...
            sql = "SELECT * FROM motor WHERE jenis_mtr LIKE %s"
            val = (f"%{jenis_mtr}%",)
            ...
            if not results:
                QMessageBox.information(self.centralwidget, "Data Tidak Ditemukan", "Motor dengan jenis tersebut tidak ditemukan.")
    ...
    def isi_form_dari_tabel(self):
        current_row = self.motor_table.currentRow()
        
        # Pastikan ada baris yang dipilih
        if current_row >= 0:
            # Ambil data dari setiap kolom
            id_mtr = self.motor_table.item(current_row, 0).text()
            jenis_mtr = self.motor_table.item(current_row, 1).text()
            type_mtr = self.motor_table.item(current_row, 2).text()
            tahun_mtr = self.motor_table.item(current_row, 3).text()
            warna_mtr = self.motor_table.item(current_row, 4).text()
            stok_mtr = self.motor_table.item(current_row, 5).text()
            harga_sewa_mtr = self.motor_table.item(current_row, 6).text()
            
            # Isi form dengan data yang dipilih
            self.lineEdit_id.setText(id_mtr)
            self.lineEdit_jenis.setText(jenis_mtr)
            self.lineEdit_type.setText(type_mtr)
            self.lineEdit_tahun.setText(tahun_mtr)
            self.lineEdit_warna.setText(warna_mtr)
            self.lineEdit_stok.setText(stok_mtr)
            self.lineEdit_harga.setText(harga_sewa_mtr)
            ...
            try:
                ...
                mycursor.execute("SELECT foto FROM motor WHERE id_mtr = %s", (id_mtr,))
    ...
    def update_motor_data(self):
        try:
            ...
            id_mtr = self.lineEdit_id.text()  
            jenis_mtr = self.lineEdit_jenis.text()  
            type_mtr = self.lineEdit_type.text()  
            tahun_mtr = self.lineEdit_tahun.text()
            warna_mtr = self.lineEdit_warna.text()  
            stok_mtr = int(self.lineEdit_stok.text())  
            harga_sewa_mtr = int(self.lineEdit_harga.text())  

            # Validasi input tidak boleh kosong
            if not all([id_mtr, jenis_mtr, type_mtr, tahun_mtr, warna_mtr, stok_mtr, harga_sewa_mtr]):
            ...
            # Query SQL untuk memperbarui data motor
            if foto_path:
                sql = """
                UPDATE motor
                SET jenis_mtr = %s, type_mtr = %s, tahun_mtr = %s, 
                    warna_mtr = %s, stok_mtr = %s, harga_sewa_mtr = %s, foto = %s
                WHERE id_mtr = %s
                """
                val = (jenis_mtr, type_mtr, tahun_mtr, warna_mtr, stok_mtr, harga_sewa_mtr, foto_path, id_mtr)
            else:
                sql = """
                UPDATE motor
                SET jenis_mtr = %s, type_mtr = %s, tahun_mtr = %s, 
                    warna_mtr = %s, stok_mtr = %s, harga_sewa_mtr = %s
                WHERE id_mtr = %s
                """
                val = (jenis_mtr, type_mtr, tahun_mtr, warna_mtr, stok_mtr, harga_sewa_mtr, id_mtr)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton_cari.clicked.connect(self.cari_motor)
        self.pushButton_simpan.clicked.connect(self.update_motor_data)
        self.pushButton_keluar.clicked.connect(self.KembaliMenu)
        self.pushButton_keluar.clicked.connect(MainWindow.close)
