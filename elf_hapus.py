...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        # Title
        title_label = QtWidgets.QLabel("HAPUS DATA ELF")
        ...
        # Foto Label
        self.label_foto = QtWidgets.QLabel("Foto elf")
        ...
        self.pushButton_hapus = QtWidgets.QPushButton("Hapus elf")
        ...
        # Connect buttons
        self.pushButton_hapus.clicked.connect(self.delete_elf)
        self.pushButton_refresh.clicked.connect(self.load_elf_data)
        self.pushButton_keluar.clicked.connect(self.KembaliMenu)
        
        # Connect selection changed
        self.tableView.selectionModel().selectionChanged.connect(self.show_elf_details)
        
        # Initial data load
        self.load_elf_data()

    def load_elf_data(self):
    ...
    def show_elf_details(self):
        # Clear previous photo
        self.label_foto.clear()
        self.label_foto.setText("Foto elf")
        ...
        row = indexes[0].row()
        id_elf = self.model.item(row, 0).text()
        ...
        try:
            ...
            sql = "SELECT foto FROM elf WHERE id_elf = %s"
            val = (id_elf,)
    ...
    def delete_elf(self):
        ...
        indexes = self.tableView.selectedIndexes()
        if not indexes:
            QMessageBox.warning(None, "Peringatan", "Pilih elf yang akan dihapus!")
            return
        ...
        reply = QMessageBox.question(
            None, 
            'Konfirmasi Hapus', 
            f'Apakah Anda yakin ingin menghapus elf dengan ID {id_elf}?',
        ...
        try:
            ...
            sql_foto = "SELECT foto FROM elf WHERE id_elf = %s"
            mycursor.execute(sql_foto, (id_elf,))
            foto_result = mycursor.fetchone()

            # Hapus data dari database
            sql_delete = "DELETE FROM elf WHERE id_elf = %s"
            mycursor.execute(sql_delete, (id_elf,))
            conn.commit()
            ...
            QMessageBox.information(None, "Sukses", f"elf dengan ID {id_elf} berhasil dihapus!")
