...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow, motor_id=None, user_id=None):
        ...
        self.label_jumlah.setText("Jumlah Motor yang Disewa:")
        ...
        # If motor_id is provided, automatically load details
        if motor_id:
            self.lineEdit_id.setText(str(motor_id))
            self.cari_motor_auto()

    def cari_motor_auto(self):
        try:
            # Get the motor ID entered by the user
            id_mtr = int(self.lineEdit_id.text())
            ...
            # Query to get motor details
            query = """
            SELECT harga_sewa_mtr, driver, stok_mtr 
            FROM motor 
            WHERE id_mtr = %s
            """
            cursor.execute(query, (id_mtr,))
            result = cursor.fetchone()

            if result:
                # Update UI with motor details
                ...
                # Store motor_id for later use
                self.motor_id = id_mtr
            else:
    ...
    def simpan_data(self):
        try:
            ...
            jumlah_disewa = self.spinBox_jumlah.value()  # Jumlah motor yang disewa
            ...
            # Ambil detail motor berdasarkan ID yang dipilih
            query_motor = """
            SELECT m.id_mtr, m.harga_sewa_mtr, m.driver, m.stok_mtr
            FROM motor m
            WHERE m.id_mtr = %s AND m.stok_mtr >= %s
            """
            cursor.execute(query_motor, (self.motor_id, jumlah_disewa))
            ...
            if motor_result:
                ...
                total_harga = harga_sewa * jumlah_disewa
                ...
                WHERE id_mtr = %s
                """
                cursor.execute(query_update_stok, (jumlah_disewa, id_motor))

                conn.commit()  # Commit untuk menyimpan perubahan ke database

                # Tampilkan pesan sukses
                QtWidgets.QMessageBox.information(None, "Sukses", "Penyewaan motor berhasil!")
