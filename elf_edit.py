...
# Form implementation generated from reading ui file 'edit_elf.ui'
#
...
"background-color: rgb(212, 17, 30);")
    ...
    def cari_elf(self):
        ...
        try:
            ...
            # Query SQL untuk mencari elf berdasarkan ID
            sql = "SELECT * FROM elf WHERE id_elf = %s"
            val = (id_elf,)
            ...
            if result:
                ...
                self.lineEdit_jenis.setText(str(result[1]))  # jenis_elf
                self.lineEdit_type.setText(str(result[2]))  # type_elf
                self.lineEdit_tahun.setText(str(result[3]))  # tahun_elf
                self.lineEdit_warna.setText(str(result[4]))  # warna_elf
                self.lineEdit_stok.setText(str(result[5]))  # stok_elf
                self.lineEdit_harga.setText(str(result[6]))  # harga_sewa_elf
            else:
                QtWidgets.QMessageBox.information(self.centralwidget, "Data Tidak Ditemukan", "Elf dengan ID tersebut tidak ditemukan.")
    ...
    def update_elf_data(self):
        ...
        id_elf = self.lineEdit_id.text()  # Ambil input ID Elf
        jenis_elf = self.lineEdit_jenis.text()  # Ambil input Jenis Elf
        type_elf = self.lineEdit_type.text()  # Ambil input Tipe Elf
        tahun_elf = self.lineEdit_tahun.text()
        warna_elf = self.lineEdit_warna.text()  # Ambil input Warna Elf
        stok_elf = int(self.lineEdit_stok.text())  # Ambil input Stok Elf
        harga_sewa_elf = int(self.lineEdit_harga.text())  # Ambil input Harga Sewa
        ...
        try:
            ...
            sql = """
            UPDATE elf
            SET jenis_elf = %s, type_elf = %s, tahun_elf = %s, warna_elf = %s, stok_elf = %s, harga_sewa_elf = %s
            WHERE id_elf = %s
            """
            ...
            # Commit perubahan
            conn.commit()
            print("Data elf berhasil diperbarui...")
