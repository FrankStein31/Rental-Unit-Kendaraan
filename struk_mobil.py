...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow, user_id=None):
        ...
        header_label = QtWidgets.QLabel("Struk Transaksi Mobil")
        ...
        self.transaction_table.setHorizontalHeaderLabels(["ID", "Jenis Mobil", "Tipe Mobil", "Tahun", "Warna", "Total Pembayaran"])
    ...
    def load_all_transactions(self):
        try:
            ...
            query = """
                SELECT DISTINCT m.id_mbl, m.jenis_mbl, m.type_mbl, m.tahun_mbl, m.warna_mbl, pm.total_pembayaran
                FROM mobil m
                JOIN pinjam p ON m.id_mbl = p.id_mobil
                JOIN pembayaran pm ON p.id_transaksi = pm.id_transaksi
                WHERE p.id_user = %s
            """
    ...
    def show_transaction_detail(self):
        ...
        data_message = f"""
        ID Mobil: {details[0]}
        Jenis Mobil: {details[1]}
        Tipe Mobil: {details[2]}
        Tahun Mobil: {details[3]}
        Warna Mobil: {details[4]}
