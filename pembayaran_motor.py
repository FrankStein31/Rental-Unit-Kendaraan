...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow, user_id=None):
        ...
        self.label_title.setText("Pembayaran Motor")
    ...
    def load_latest_transaction(self):
        try:
            ...
            query = """
            SELECT p.*, m.harga_sewa_mtr 
            FROM pinjam_motor p
            JOIN motor m ON p.id_motor = m.id_mtr
            WHERE p.id_user = %s 
            ORDER BY p.id_transaksi_motor DESC 
            LIMIT 1
            """
            ...
            if transaction:
                ...
                self.label_total_sewa.setText(f"Total Biaya Sewa: Rp {total_sewa:,.2f}")
                ...
                self.current_transaction_id = transaction['id_transaksi_motor']
            else:
    ...
    def bayar_transaksi(self):
        try:
            ...
            harga_sewa_per_hari = transaksi['harga_sewa_mtr']
            ...
            query_insert = """
            INSERT INTO pembayaran_motor 
            (id_transaksi_motor, id_user, tanggal_dikembalikan, 
