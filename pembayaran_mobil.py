...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow, user_id=None):
        ...
        print("Ini id user dari HALAMAN PEMBAYARAN MOBIL", user_id)
...
"background-color: rgb(212, 17, 30);")
    ...
    def hitung_denda(self):
        try:
            ...
            query_pinjam = """
            SELECT p.tanggal_pinjam, m.harga_sewa_mbl
            FROM pinjam p
            ...
            if pinjam_result:
                ...
                query_insert = """
                INSERT INTO pembayaran (id_transaksi, id_user, tanggal_dikembalikan, denda, total_pembayaran)
                VALUES (%s, %s, %s, %s, %s)
                """
                cursor.execute(query_insert, (
                    id_transaksi, 
                    id_user, 
                    tanggal_dikembalikan.toString('yyyy-MM-dd'), 
                    denda, 
                    total_pembayaran
                ))
                conn.commit()
